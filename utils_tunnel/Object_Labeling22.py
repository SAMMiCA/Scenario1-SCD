...
def ObjectLabeling(client,map_id,name2idx):
    ...
    # for electric tunnel
    found = client.simSetSegmentationObjectID("[\w]*floor[\w]*", 1, True)
    found = client.simSetSegmentationObjectID("[\w]*Floor[\w]*", 1, True)
    found = client.simSetSegmentationObjectID("[\w]*Cube[\w]*", 1, True)

    found = client.simSetSegmentationObjectID("[\w]*wall[\w]*", 2, True)
    found = client.simSetSegmentationObjectID("[\w]*Wall[\w]*", 2, True)

    found = client.simSetSegmentationObjectID("[\w]*ceiling[\w]*", 3, True)
    found = client.simSetSegmentationObjectID("[\w]*Ceiling[\w]*", 3, True)

    found = client.simSetSegmentationObjectID("[\w]*pipe[\w]*", 4, True)
    found = client.simSetSegmentationObjectID("[\w]*Pipe[\w]*", 4, True)

    found = client.simSetSegmentationObjectID("[\w]*box[\w]*", 5, True)


    found = client.simSetSegmentationObjectID("[\w]*extinguisher[\w]*", 6, True)
    found = client.simSetSegmentationObjectID("[\w]*Extinguisher[\w]*", 6, True)
    found = client.simSetSegmentationObjectID("[\w]*fire_ex[\w]*", 6, True)

    found = client.simSetSegmentationObjectID("[\w]*wallbox[\w]*", 7, True)
    found = client.simSetSegmentationObjectID("[\w]*switch[\w]*", 7, True)

    found = client.simSetSegmentationObjectID("[\w]*door[\w]*", 8, True)
    found = client.simSetSegmentationObjectID("[\w]*Door[\w]*", 8, True)
    found = client.simSetSegmentationObjectID("[\w]*doorframe[\w]*", 8, True)

    found = client.simSetSegmentationObjectID("[\w]*sign[\w]*", 9, True)
    found = client.simSetSegmentationObjectID("[\w]*Sign[\w]*", 9, True)

    found = client.simSetSegmentationObjectID("[\w]*light[\w]*", 10, True)
    found = client.simSetSegmentationObjectID("[\w]*tunnel_light[\w]*", 10, True)
    found = client.simSetSegmentationObjectID("[\w]*lamp[\w]*", 10, True)

    found = client.simSetSegmentationObjectID("[\w]*ladder[\w]*", 11, True)
    found = client.simSetSegmentationObjectID("[\w]*Ladder[\w]*", 11, True)

    found = client.simSetSegmentationObjectID("[\w]*Tank[\w]*", 12, True)

    found = client.simSetSegmentationObjectID("[\w]*barrel[\w]*", 13, True)
    found = client.simSetSegmentationObjectID("[\w]*barel[\w]*", 13, True)
    found = client.simSetSegmentationObjectID("[\w]*Barrel[\w]*", 13, True)

    found = client.simSetSegmentationObjectID("[\w]*SM_train[\w]*", 14, True)
